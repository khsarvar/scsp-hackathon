...
Flow:
  ...
  5. Return everything (cleaning steps + agent steps + final findings) so the UI
     can show the actual code, output, and chart PNGs.
...
def run_analysis(req: AnalyzeRequest):
    ...
    # Light deterministic clean so the agent doesn't waste calls on basic fixes.
